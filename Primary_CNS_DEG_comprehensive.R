...
batchType = c(rep(1,6), rep(2,60), rep(3,20), rep(4,22))
modType = c(gl3521, gl14682, gl14683, gl63668)
...
load("group_list.Rdata")
library(limma)
group_list = c(gl3521, gl14682, gl14683, gl63668)
...
write.csv(sfg, file = "D:\\STUDY\\szbl\\metastatic cell genomic\\results\\Primary_CNS\\deg analysis\\DEG\\sfg_composed.csv")
...
annotation_col = data.frame(sampletype = c(gl3521, gl14682, gl14683, gl63668))
...
## If Excluding double-channel data ##
...
data14683$symbol = rownames(data14683)
data63668$symbol = rownames(data63668)
merge_eset = inner_join(data14682,data14683, by = "symbol")
dim(merge_eset)
merge_eset = inner_join(merge_eset,data63668, by = "symbol")
dim(merge_eset)
rownames(merge_eset) = merge_eset$symbol
merge_eset = merge_eset[, c(-61)]
...
batchType = c(rep(1,60), rep(2,20), rep(3,22))
modType = c(gl14682, gl14683, gl63668)
mod = model.matrix(~as.factor(modType))
outTab = data.frame(ComBat(data, batchType, mod, par.prior = TRUE))
boxplot(outTab)
...
# load("group_list.Rdata")
library(limma)
group_list = c(gl14682, gl14683, gl63668)
design <- model.matrix(~0+factor(group_list))
colnames(design) = levels(factor(group_list))
rownames(design) = colnames(exp)
